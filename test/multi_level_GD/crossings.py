...
def count_crossings(G, edges_to_compare=None, stop_when_found=False, ignore_label_edge_cr=False):
    ...
    for c1 in range(0, len(edge_set_1)):
        ...
        for c2 in range(j_start, len(edge_list)):
            ...
            #if(intersection_exists):
            if(intersection_exists and (intersection_point!=None)):
...
def count_crossings_smooth(G, edges_to_compare=None, stop_when_found=False, ignore_label_edge_cr=False):
    #obj = 0
    #for all pair of edges
    #  if the edge pair is parallel
    #    if they are intersecting
    #      take the mid point of the intersection region
    #    else they don't intersect
    #      the intersecting point is at infinity
    #  else they are not parallel
    #    find the intersecting point of the corresponding line
    #  compute the distance of four end points from the intersecting point
    #  let d_min = minimum of four distances
    ...
    for c1 in range(0, len(edge_set_1)):
        ...
        for c2 in range(j_start, len(edge_list)):
            ...
            if(intersection_exists):
                (intersection_x, intersection_y) = intersection_point
                d_min = min(distance(intersection_x, intersection_y, p1x, p1y), distance(intersection_x, intersection_y, q1x, q1y), distance(intersection_x, intersection_y, p2x, p2y), distance(intersection_x, intersection_y, q2x, q2y))
                try:
                  f_2 = 1/(1+math.exp(d_min))
                ...
                (intersection_x, intersection_y) = compute_intersection_point(p1x, p1y, q1x, q1y, p2x, p2y, q2x, q2y)
                d_min = -min(distance(intersection_x, intersection_y, p1x, p1y), distance(intersection_x, intersection_y, q1x, q1y), distance(intersection_x, intersection_y, p2x, p2y), distance(intersection_x, intersection_y, q2x, q2y))
                try:
                  f_2 = 1/(1+math.exp(d_min))
                except OverflowError:
